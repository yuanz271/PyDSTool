...
class Interval(object):
    ...
    def __call__(self):
        if self.defined:
            return (self.name, self.type, self.typestr, (self._loval, self._hival))
        else:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self._loval == other._loval
            and self._hival == other._hival
            and self._abseps == other._abseps
        )
    ...
    def __contains__(self, val):
        ...
        if testresult == notcontained:
            ...
            raise PyDSTool_UncertainValueError(
                "cannot determine membership " "(uncertain)", val
            )
        ...
        try:
            ...
            if isinstance(val, _num_name2equivtypes[self.typestr]):
                ...
                if isinstance(val, _all_int):
                    ...
                    if not compareNumTypes(val.type, self.type) and compareNumTypes(
                        val.type, _all_float
                    ):
                        # meaningless to ask if float interval is contained in an
                        # integer interval!
                        raise PyDSTool_TypeError("Interval type mismatch")
                    if compareNumTypes(val.type, self.type) and compareNumTypes(
                        self.type, _all_int
                    ):
                        eps = 0
                    else:
                    ...
                    if tempIlo and tempIhi:
                        ...
                        if isfinite(val._hival) or isfinite(self._hival):
                            ...
                            tempEhi = False
                        if (
                            (tempUlo and not tempEhi) or (tempUhi and not tempElo)
                    ...
                    if compareNumTypes(val.type, self.type):
                        ...
                        if compareNumTypes(val.type, _all_int):
                            ...
                            if (
                                int(val._loval) == val._loval
                                and int(val._hival) == val._hival
                            ):
            ...
            raise PyDSTool_TypeError(
                "Expected a numeric type or a singleton "
    ...
    def intersect(self, other):
        if not isinstance(other, Interval):
            raise PyDSTool_TypeError("Can only intersect with other Interval " "types")
        result = None  # default, initial value if no intersection
        if self.type != other.type:
            raise PyDSTool_TypeError(
                "Can only intersect with other Intervals " "having same numeric type"
            )
        if compareNumTypes(self.type, _all_complex) or compareNumTypes(
            other.type, _all_complex
        ):
        ...
        if self.contains(other):
            ...
            if other.contains(self._hival) is contained:
                ...
                result = Interval("__result__", self.type, [other._loval, self._hival])
            elif other.contains(self._loval) is contained:
                # then also self.contains(other._hival)
                result = Interval("__result__", self.type, [self._loval, other._hival])
        return result
    ...
    def atEndPoint(self, val, bdcode):
        ...
        assert isinstance(val, (_int_types, _float_types)), "Invalid value type"
    ...
    """
        ...
        if compareNumTypes(self.type, _all_float):
            if strict:
                ...
                samplelist = list(arange(self._loval, self._hival, dt, dtype=float))
            ...
            if not isinstance(dt, _int_types):
                raise ValueError("dt must be an integer for integer " "intervals")
            if strict:
                print(
                    "Warning: 'strict' option is invalid for integer "
                    + "interval types"
                )
            if avoidendpoints:
                loval = self._loval + 1
                hival = self._hival - 1
                assert loval <= hival, (
                    "There are no points to return with " "these options!"
                )
            else:
                loval = self._loval
                hival = self._hival
    ...
    def set(self, arg):
        ...
        if isinstance(arg, _seq_types) and len(arg) == 2:
            if arg[0] == arg[1]:
                ...
                if not loval < hival:
                    print("set() was passed loval = ", loval, " and hival = ", hival)
                    raise PyDSTool_ValueError(
                        "Interval endpoints must be "
                        "given in order of increasing size"
                    )
                self._intervalstr = "[" + str(loval) + "," + str(hival) + "]"
            ...
            assert isfinite(arg), "Singleton interval domain value must be finite"
            if self.isdiscrete:
