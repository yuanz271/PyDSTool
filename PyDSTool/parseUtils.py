...
def mapPowStr(t, p="**"):
    ...
    if p == "**":
        lpart = dopower(
            ensureparen(ast2string(toDoubleStarSyntax(string2ast(ll[0]))), 1),
        ...
        if len(t) > 3:
            res = ensureparen(
                ast2string(
                    toDoubleStarSyntax(["power", string2ast("__LPART__")] + t[3:])
                )
            )
        ...
        lpart = dopower(
            ensureparen(ast2string(toCircumflexSyntax(string2ast(ll[0]))), 1),
        ...
        if len(t) > 3:
            res = ensureparen(
                ast2string(
                    toCircumflexSyntax(["power", string2ast("__LPART__")] + t[3:])
                ),
                1,
            )
            return res.replace("__LPART__", lpart)
        ...
        lpart = dopower(
            ensurebare(ast2string(toPowSyntax(string2ast(ll[0])))),
            ensurebare(ast2string(toPowSyntax(string2ast(ll[1])))),
            "pow(%s,%s)",
        )
        if len(t) > 3:
            res = ensurebare(
                ast2string(toPowSyntax(["power", string2ast("__LPART__")] + t[3:]))
            )
            return res.replace("__LPART__", lpart)
        else:
            return ensureparen(lpart, 1)
        ...
        if t[0] == "power":
            if t[2][0] == "DOUBLESTAR":
                return string2ast(
                    ensureparen(
                        dopower(
                            ast2string(toCircumflexSyntax(t[1])),
            ...
            try:
                if t[2][0] == "DOUBLESTAR":
                    try:
                        return string2ast(
                            dopower(
                                ensurebare(ast2string(toPowSyntax(t[1]))),
                    ...
                    try:
                        return string2ast(
                            dopower(
                                ensurebare(ast2string(toPowSyntax(t[1:3]))),
...
def convertPowers(s, target="pow"):
    ...
    if target == "**":
        ...
        try:
            ...
            s = temp_macro_names(s)
            return temp_macro_names_inv(
                ast2string(ensureints(toCircumflexSyntax(string2ast(s))))
            )
    elif target == "pow":
        try:
...
def ast2shortlist(t):
    if type(t) is parser.STType:
        return ast2shortlist(t.tolist())
...
def dopower(l, r, pow_str=POW_STR):
    ...
    if pow_str == "%s**%s":
        ...
        return trysimple(
            "%s^%s"
            % (ensuredecimalconst(ensureparen(l)), ensuredecimalconst(ensureparen(r)))
        )
    elif pow_str == "pow(%s,%s)":
        return trysimple(
            "pow(%s,%s)" % (l, r)
        )  # trysimple('pow(%s,%s)'%(ensurebare(l),ensurebare(r)))
    else:
        raise ValueError("Invalid target power syntax")


def domul(l, r):
    if l in ZEROS or r in ZEROS:
        return "0"
    if l in ONES:
        return r
    if r in ONES:
        return l
    if l in ["-" + o for o in ONES]:
        return doneg(r)
    if r in ["-" + o for o in ONES]:
        return doneg(l)
    lft = string2ast(l)
    rt = string2ast(r)
    ...
    if lft_neg and rt_neg or not (lft_neg or rt_neg):
        return trysimple(
            "%s*%s" % (ensureparen(new_l, ismul=1), ensureparen(new_r, ismul=1))
        )
    else:
        return trysimple(
            "-%s*%s" % (ensureparen(new_l, ismul=1), ensureparen(new_r, ismul=1))
...
def dodiv(l, r):
    ...
    if lft_neg and rt_neg or not (lft_neg or rt_neg):
        return trysimple(
            "%s/%s"
            % (
                ensureparen(ensuredecimalconst(new_l), ismul=1, do_decimal=DO_DEC),
...
def dosub(l, r):
    ...
    if l == r:
        return "0"
    if r[0] == "-":
        return ensureparen(trysimple("%s+%s" % (l, doneg(r))))
    return trysimple("%s-%s" % (l, r))
...
def ensureparen_div(tt):
    if tt[0] == "term" and tt[2][0] == "SLASH" and len(tt[3:]) > 1:
        return [
            "term",
            string2ast(ensureparen(ast2string(tt[1]) + "/" + ast2string(tt[3]), 1)),
        ] + tt[4:]
    else:
        return tt
    ...
    if t[0] == "arith_expr":
        ...
        for nix in num_ixs:
            if ops[nix] > 0:
                ...
                res_denominator = domul(
                    res_denominator, ast2string(simplify(args[nix]))
                )
        # follow by other terms
        for oix in oth_ixs:
            if ops[oix] > 0:
                ...
                res_denominator = domul(
                    res_denominator, ast2string(simplify(args[oix]))
                )
    ...
    def __call__(self, arg):
        ...
        if isinstance(arg, str):
            ...
            try:
                ...
                raise TypeError(
                    "symbolMapClass does not know how to "
...
class auxfnDBclass(object):
    ...
    def addAuxFn(self, auxfnName, parserObj):
        if parserObj not in self.auxnames:
            ...
            raise ValueError(
                "Parser object " + parserObj.name + " already "
...
class parserObject(object):
    ...
    def parse(self, specialtoks, symbolMap=None, includeProtected=True, reset=False):
        if reset:
            self.usedSymbols = []
            self.freeSymbols = []
        if symbolMap is None:
        ...
        while scount < speclen:
            ...
            if name_chars_RE.match(stemp) is None:
                ...
                if s not in ["", " ", "\n", "\t"]:
                    ...
                    if s in allnames:
                        ...
                        if isdecimal or (ishierarchicalname and dohierarchical):
                            ...
                            snew = symbolMap(s)
                            if s[0] not in num_chars + ["+", "-"] and snew not in free:
                                free.append(snew)
                            tokenized.append(snew)
                            if snew not in used:
                                used.append(snew)
                ...
                if stemp in ["+", "-"]:
                    ...
                    try:
                        ...
                        continue
                    if (
                        tokenized == [] or tokenized[-1] == "("
                    ) and next_stemp in num_chars + ["."]:
                    ...
                    if (
                        ...
                        and (
                            ...
                            or (
                                "[" in specialtokens
                                and not (
                                    isVectorClause(specstr[scount - 1 :])
                                    or len(tokenized) > 1
                                    and tokenized[-2] in ("max", "min", "max_", "min_")
                                )
                            )
                        )
                    ):
                        ...
                        raise ValueError(
                            "Syntax error: Square braces not to "
            ...
            if s in specialtokens:
                ...
                if (
                    s == "["
                    and self.treatMultiRefs
                    and len(tokenized) > 0
                    and (
                        tokenized[-1].isalnum()
                        or (
                            "[" in specialtokens
                            and not isVectorClause(specstr[scount - 1 :])
                        )
                    )
                ):
                    # then this is probably an actual multiRef ...
                    ...
                    temp = parserObject(expr[1:-1], includeProtected=False)
                    ...
                    if scount < speclen - 1:
                        if name_chars_RE.match(specstr[scount]) is None:
                            ...
                            if (
                                s[-1] in ["e", "E"]
                                and s[0] not in ["e", "E"]
                                and name_chars_RE.match(specstr[scount]).group()
                                in num_chars + ["-", "+"]
                            ):
                    ...
                    if includeProtected:
                        if s == "for":
                            ...
                            try:
                                ...
                                raise ValueError("Mismatch ( and ) in 'for' " "macro")
                            # expr includes both brackets
                            ...
                            if not alltrue(macrotests):
                                ...
                                raise ValueError("Invalid sub-clause in " "'for' macro")
                            # start next parsing iteration after the ')'
                            scount += rbpos + 1
...
def isToken(s, treatMultiRefs=False):
    ...
    if not isinstance(s, str):
        return False
    try:
        temp = parserObject(s, includeProtected=False, treatMultiRefs=treatMultiRefs)
    except ValueError:
        return False
    ...
    return (
        not temp.isCompound()
        and len(temp.usedSymbols) == lenval
...
def isNameToken(s, treatMultiRefs=False):
    return (
        isToken(s, treatMultiRefs=treatMultiRefs)
        and s[0] not in num_chars
        and not (len(s) == 1 and s == "_")
    )
...
def isHierarchicalName(s, sep=NAMESEP, treatMultiRefs=False):
    s_split = s.split(sep)
    return len(s_split) > 1 and alltrue(
        [isNameToken(t, treatMultiRefs) for t in s_split]
    )
...
def replaceSep(spec, sourcesep=NAMESEP, targetsep="_"):
    ...
    try:
        assert all([alphanumeric_chars_RE.match(char) is None for char in sourcesep])
    except AssertionError:
        raise ValueError("Source separator must be non-alphanumeric")
    try:
        assert all([alphanumeric_chars_RE.match(char) is None for char in targetsep])
    except AssertionError:
...
def replaceSepStr(spec, sourcesep, targetsep):
    ...
    p = parserObject(spec, treatMultiRefs=treatMultiRefs, includeProtected=False)
    # search for pattern <nameToken><sourcechar><nameToken>
...
def parseMatrixStrToDictStr(specstr, specvars, m=0):
    ...
    try:
        while not splitdone:
            nextrbrace = findEndBrace(spectemp2[startpos:], "[", "]") + startpos
            if nextrbrace is None:
                raise ValueError("Mismatched braces before end of string")
            specdict[specvars[entrycount]] = spectemp2[startpos : nextrbrace + 1]
            entrycount += 1
            if entrycount < n:
...
def wrapArgInCall(source, callfn, wrapL, wrapR=None, argnums=[0], notFirst=False):
    ...
    if wrapR is None:
        ...
        wrapR = wrapL
    assert isinstance(wrapL, str) and isinstance(
        wrapR, str
...
def replaceCallsWithDummies(source, callfns, used_dummies=None, notFirst=False):
    ...
    while not done:
        # find any callfns in source (now just always a singleton)
        findposlist_candidates = [
            source[currpos:].find(fname + "(") for fname in callfns
        ]
        findposlist = []
        for candidate_pos in findposlist_candidates:
            # remove any that are actually only full funcname matches to longer strings
        ...
        if not done:
            ...
            try:
                ...
                if subpositions == []:
                    ...
                    filtered_sub_source, new_d = replaceCallsWithDummies(
                        sub_source, callfns, used_dummies
                    )
            except ValueError:
                pass
            else:
                if new_d != {}:
                    ...
                    used_dummies = max((used_dummies, max(dummies.keys())))
            used_dummies += 1
            dummies[used_dummies] = (
                source[findpos : startbrace + 1] + filtered_sub_source + ")"
            )
            output += source[currpos:findpos] + "__dummy%i__" % used_dummies
            currpos = endbrace + 1
        else:
    ...
    if dummies != {}:
        new_dummies = dummies.copy()
        for k, v in dummies.items():
            new_v, new_d = replaceCallsWithDummies(
                v, callfns, used_dummies, notFirst=True
            )
            new_dummies[k] = new_v
            if new_d != {}:
                new_dummies.update(new_d)
        ...
        if not done:
            ...
            try:
                ...
                if subpositions == []:
                    ...
                    filtered_sub_source = addArgToCalls(
                        sub_source, callfns, arg, notFirst
                    )
            except ValueError:
                pass
            ...
            if currpos == 0 and callfns == [notFirst]:
                ...
                if filtered_sub_source == "":
                    ...
                    addStr = ", " + arg
            output += (
                source[currpos : startbrace + 1] + filtered_sub_source + addStr + ")"
            )
            currpos = endbrace + 1
        else:
            output += source[currpos:]
    ...
    while ix < len(specstr) and term_len > 0:
        ...
        if found_ix > -1:
            ...
            if isNumericToken(term):
                ...
                if specstr[pos - 1] in num_chars + [".", "e"] or specstr[
                    pos + term_len
                ] in num_chars + [".", "e"]:
                    ix = pos + term_len
                    continue
