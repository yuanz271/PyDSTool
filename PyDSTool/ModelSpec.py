...
_classes = ["ModelSpec", "Component", "LeafComponent", "nameResolverClass"]

_functions = ["searchModelSpec", "matchSubName", "processMacro", "resolveMacroTargets"]
...
class ModelSpec(object):
    ...
    def flattenSpec(
        self, multiDefUnravel=True, globalRefs=None, ignoreInputs=False, force=False
    ):
       ...
       """
        ...
        if not self.isComplete(globalRefs):
            ...
            raise ValueError("Cannot flatten incomplete functional " "specification")
        ...
        if not ignoreInputs:
            for i in self.inputs.values():
                if not i.isDefined(verbose):
                    if verbose:
                        print(
                            "... in '%s' (type %s)" % (self.name, str(self.__class__))
                        )
                        return False
        ...
        if isinstance(arg, list):
            ...
            if isinstance(obj, Var):
                ...
                if not issubclass(self.__class__, obj.compatibleContainers):
                    ...
                    print(
                        "  with compatible container types:%r"
                        % obj.compatibleContainers
                    )
                    print(
                        "Compatible sub-component types: %r"
                        % self._allSubcomponentTypes
                    )
                    raise ValueError(
                        "Incompatible sub-component type"
                        " for object '" + objname + "'"
                    )
                else:
    ...
    def _register(self, obj, depth=0, parent_obj=None):
        ...
        if len(obj.compatibleGens) == 0:
            ...
            raise ValueError(
                "Incompatible generators found in component" " '" + obj.name + "'"
            )
        if len(obj.targetLangs) == 0:
            ...
            raise ValueError(
                "Incompatible target language in component" " '" + obj.name + "'"
            )
        ...
        if not compareClassAndBases(obj, self._allSubcomponentTypes):
            print(
                "Valid sub-component types that have been declared:%r"
                % self._allSubcomponentTypes
            )
            raise TypeError(
        ...
        if isMultiDefClash(obj, self.multiDefRefs, parentname):
            raise ValueError(
                "Object %s defines names that already exist in" " registry" % obj.name
            )
        #        # TEMP
        #        print "\n------------------------"
        ...
        if hasattr(obj, "_registry"):
            ...
            self.freeSymbols = remain(self.freeSymbols, globalized_newdefs)
            ...
            for sub_regObj in obj._registry.values():
                ...
                self._registry[subobjname] = regObject(new_regObj.obj, actual_parent)
            ...
            try:
                ...
                raise TypeError(
                    "Invalid object type in _register(): %s (type"
            ...
            if objname in self._registry:
                raise ValueError(
                    "Name '%s' already exists in " % objname
        ...
        if do_obj_register:
            self._registry[objname] = regObject(deepcopy(obj), parent_obj or self)
        ...
        if do_free_symbs:
            ...
            self.freeSymbols.extend(
                remain(obj_freeSymbols, list(self._registry.keys()) + self.freeSymbols)
            )
    ...
    def remove(self, target):
       ...
       Use global hierarchical names for components if specifying a string."""
        ...
        if hasattr(target, "name"):
            ...
            raise TypeError("Invalid type to remove from ModelSpec " "'%s'" % self.name)
        try:
        ...
        if isHierarchicalName(objname):
            ...
            try:
                ...
                print(e)
                raise ValueError(
                    "Error recognizing parent object '%s' in "
            ...
            if objname in self.variables:
                ...
                raise ValueError(
                    "%s: Error recognizing object '%s'" % (self.name, objname)
                )
    ...
    def isComplete(self, globalRefs=None):
        if globalRefs is None:
            globalRefs = allmathnames_symbolic + ["True", "False", "and", "or", "not"]
        else:
            globalRefs.extend(
                allmathnames_symbolic + ["True", "False", "and", "or", "not"]
            )
    ...
    def _infostr(self, verbose=1):
        ...
        if verbose > 0:
            if len(self.components) > 0:
                ...
                base_str += (
                    ", ".join(
                        [c._infostr(verbose - 1) for c in self.components.values()]
                    )
                    + " )"
                )
            ...
            if regObj.namemap != {}:
                if isinstance(add_obj, Fun):
                    ...
                    for name in regObj.namemap:
                        ...
                        if mappedName in add_obj.freeSymbols and not isinstance(
                            self._registry[mappedName].obj, (Par, Fun)
                        ):
                            print(
                                "Problem with registry object:%r"
                                % self._registry[mappedName].obj
                            )
                            print("in auxiliary function:" + add_obj.name)
                            print("   that has free symbols:%r" % add_obj.freeSymbols)
                            raise TypeError(
                                "Fun '" + add_obj.name + "' "
...
class LeafComponent(ModelSpec):
    ...
    def isEmpty(self):
        return self.variables == {} and self.pars == {} and self.inputs == {}
...
Component.compatibleSubcomponents = (Par, Var, Input, Fun, Component, LeafComponent)
...
def searchModelSpec(mspec, name, component_type_order=None):
    ...
    if isHierarchicalName(name):
        ...
        if component_type_order is None or component_type_order == []:
            ...
            raise ValueError(
                "Cannot pass both a hierarchical name to search"
    ...
    if component_type_order is None or component_type_order == []:
        ...
        if name in mspec._registry:
            ...
            return [mspec._componentNameMap[o] for o in mspec._componentTypeMap[name]]
        else:
            return []
    else:
   ...
   textually into the Quantity's specification."""
    ...
    for for_ix in range(num_fors):
        ...
        if len(sourceList) == 1:
            ...
            raise ValueError(
                "source list for macro resolution should have no " "more than one entry"
            )
        targList_new, ctStr, opStr = resolveMacroTargets(
            qtemp.parser.tokenized, mspec, sourceName
        )
        ctypestrlist.append(ctStr)
        if forSubs:
            for t in targList_new:
                if t not in targList:
                    targList.append(t)
        if targList_new == []:
            specStr += "0"
        else:
    ...
    c.spec = QuantSpec(
        replaceSep(c.spec.subjectToken), specStr.strip(), c.spec.specType
    )
...
def resolveMacroTargets(toks, mspec, targParent):
    ...
    if componentTypeStr in ["Var", "Par", "Quantity"]:
        raise ValueError("Invalid component type for macro in" " %s" % targParent)
    if targParent == "":
    ...
    def getGlobalName(self):
        if self.namemap == {}:
            ...
            try:
                ...
                raise ValueError(
                    "Namemap for registry object didn't "
