...
class Python(CodeGenerator):
    ...
    def generate_aux(self):
        ...
        for auxname in auxnames:
            ...
            assert isinstance(auxinfo[0], list), (
                "aux function arguments " "must be given as a list"
            )
            ...
            if auxname == "Jacobian":
                ...
                auxstr_py = self._generate_fun(
                    "_auxfn_Jac", reusestr + body_processed, "xjac", specvars
                )
                # check Jacobian
                m = n = len(specvars)
                ...
                auxstr_py = self._generate_fun(
                    "_auxfn_Jac_p", reusestr + body_processed, "pjac", self.fspec.vars
                )
                # check Jacobian
                n = len(specvars)
                ...
                for row in range(m):
                    try:
                        if specdict_check[self.fspec.vars[row]] != n:
                            ...
                            print(
                                "Found length %i" % specdict_check[self.fspec.vars[row]]
                            )
                            raise ValueError(
                ...
                auxstr_py = self._generate_fun(
                    "_auxfn_massMatrix", reusestr + body_processed, "xmat", specvars
                )
                # check matrix
                m = n = len(specvars)
                specdict_check = {}.fromkeys(specvars)
                ...
                if badparnames != []:
                    print(
                        "Bad parameter names in auxiliary function %s: %r"
                        % (auxname, badparnames)
                    )
                    # print(str(auxinfo[0]))
        ...
        for auxname, auxspec in auxfns.items():
            dummyQ = QuantSpec(
                "dummy", auxspec[0], preserveSpace=True, treatMultiRefs=False
            )
    ...
    def generate_spec(self):
        ...
        reusestr, specupdated = self._processReusedPy(
            specname_vars, self.fspec.varspecs
        )
        ...
        specstr_py = self._generate_fun(
            "_specfn", reusestr + temp, "xnew", specname_vars, docodeinserts=True
        )
    ...
    def _specStrParse(
        ...
        if dovars:
            if forexternal:
                var_arrayixstr = dict(
                    zip(self.fspec.vars, ["'" + v + "'" for v in self.fspec.vars])
                )
                aux_arrayixstr = dict(
                    zip(self.fspec.auxvars, ["'" + v + "'" for v in self.fspec.auxvars])
                )
            else:
                var_arrayixstr = dict(
                    [(v, str(i)) for i, v in enumerate(self.fspec.vars)]
                )
                aux_arrayixstr = dict(
        ...
        if dopars:
            if forexternal:
                ...
                parsinps_arrayixstr = dict(
                    zip(parsinps_names, ["'" + pn + "'" for pn in parsinps_names])
                )
            else:
                if doinps:
                    # parsinps_names is pars and inputs, each sorted
            ...
            parsinps_arrayixstr = {}
        specialtokens = (
            remain(allnames, specials) + ["(", "t"] + remain(specials, ["t"])
        )
        specstr_lang = ""
        specname_count = 0
    ...
    def _parseReusedTermsPy(
        ...
        if dovars:
            var_arrayixstr = dict(
                zip(self.fspec.vars, map(lambda i: str(i), range(len(self.fspec.vars))))
            )
            aux_arrayixstr = dict(
                zip(
                    self.fspec.auxvars,
        ...
        if dopars:
            if doinps:
                ...
                parsinps_names = self.fspec.pars
            parsinps_arrayixstr = dict(
                zip(parsinps_names, map(lambda i: str(i), range(len(parsinps_names))))
            )
        else:
            parsinps_names = []
            parsinps_arrayixstr = {}
        ...
        for specname, itemlist in d.items():
            ...
            for strlist in itemlist:
                ...
                if len(strlist) < max(symbol_ixs):
                    raise ValueError(
                        "Symbol indices out of range in " "call to _parseReusedTermsPy"
                    )
