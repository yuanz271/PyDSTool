...
class ADMC_ODEsystem(ODEsystem):
    ...
    def __init__(self, kw):
        ...
        assert isinstance(self.funcspec, RHSfuncSpec), (
            "ADMC++ " "requires RHSfuncSpec type to proceed"
        )
        assert not self.inputs, "ADMC++ does not support external inputs feature"
        self._errorcodes = {}
        self._paraminfo = {}
        ...
        if len(self.algparams) != len(algparams_def):
            raise ValueError(
                "Invalid keys present in algparams argument: "
        ...
        if not os.path.isdir(self._compilation_tempdir):
            try:
                ...
                print(
                    "Could not create compilation temp directory "
                    + self._compilation_tempdir
                )
                raise
        ...
        if not os.path.isdir(self._target_dir):
            try:
                ...
                print("Could not creat target ADMC model directory " + self._target_dir)
                raise
    ...
    def _prepareEventSpecs(self):
        # in admc++, all events are terminal, must be 0 or 1 for direction,
        # no delay, no tolerances, etc.
        eventDir = []
        ...
        for evname in self._eventNames:
            ...
            assert isinstance(ev, MatlabEvent), (
                "ADMC++ can only " "accept matlab events"
            )

        for evname in self._eventNames:
            ev = self.eventstruct.events[evname]
            # assert ev.dircode in [-1,1],
            # ("ADMC++ requires events to have direction -1 or 1")
        ...
        for x in range(evcount):
            ...
            if self.funcspec.auxfns:
                fbody_parsed = addArgToCalls(
                    fbody, list(self.funcspec.auxfns.keys()), "p_"
                )
            else:
                fbody_parsed = fbody
    ...
    def _prepareVfieldContents(self, vfdefines):
        ...
        commentstr = (
            "% Vector field definition for model "
            + self.name
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )
    ...
    def _prepareAuxContents(self):
        ...
        if self.funcspec.auxfns:
            for finfo in self.funcspec.auxfns.values():
                ...
                fbody_parsed = addArgToCalls(
                    fbody, list(self.funcspec.auxfns.keys()), "p_", notFirst=True
                )
    ...
    def _prepareModelContents(self):
        ...
        commentstr = (
            "% Vf object definition for model "
            + self.name
            + "\n% Generated by PyDSTool for ADMC++ target\n\n"
        )
    ...
    def _prepareParamContents(self):
        ...
        if self.pars:
            ...
            bodystr += (
                str(self.pars[pnames[len(pnames) - 1]])
                + " % "
                + pnames[len(pnames) - 1]
                + " ...\n"
            )
