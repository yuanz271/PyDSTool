...
class Point(object):
    ...
    def __init__(self, kwd=None, **kw):
        ...
        if "coorddict" in kw:
            ...
            if npy.ndim(vals) > 1:
                raise ValueError(
                    "Invalid rank for coordinate array: %i" % npy.ndim(vals)
                )
            if len(vals) == 0:
                raise ValueError("Values sequence must be nonempty")
            ...
            if isinstance(vals, ndarray):
                ...
                raise TypeError(
                    "Coordinate type %s not valid for Point" % str(type(vals))
                )
        ...
        if len(coordnames) != coordvalues.shape[0]:
            ...
            print(msg)
            raise ValueError(
                "Mismatch between number of coordnames and " "dimension of data"
            )
    ...
    def _force_coords_to_ixlist(self, x):
        if x is None:
            ...
            raise ValueError(
                "Invalid coordinate / index: %s" % str(x)
    ...
    def __call__(self, coords):
        if coords in range(self.dimension + 1):
            ...
            return Point(
                {
                    "coordarray": self.coordarray[ixlist],
    ...
    def __lt__(self, other):
        try:
            ...
            return linalg.norm(self.coordarray, self._normord) < linalg.norm(
                other.coordarray, self._normord
            )
    ...
    def __copy__(self):
        return Point(
            {
                "coordarray": copy(self.coordarray),
...
class Pointset(Point):
    ...
    def __init__(self, kwd=None, **kw):
            ...
            if isinstance(vals, list):
                try:
                    ...
                    try:
                        ...
                        raise TypeError(
                            "Independent variable type %s not valid" % str(it)
                        )
                self.indepvararray = array(vals, self.indepvartype)
                ...
                raise TypeError(
                    "Invalid type for independent variable " "array: " + str(type(vals))
                )
        ...
        if self.indepvarname:
            ...
            if r == 1:
                ...
                raise ValueError(
                    "Invalid rank for " "independent variable array %i" % r
                )
        ...
        if "coorddict" in kw:
            ...
            for c in self.coordnames:
                ...
                if my_len != len(xs):
                    if self._parameterized:
                        raise ValueError(
                            "Independent variable array length must match "
            ...
            if not isinstance(kw["coordarray"], _seq_types):
                raise TypeError(
                    "Coordinate type %s not valid for Pointset"
                    % str(type(kw["coordarray"]))
                )
            try:
                ct = kw["coordtype"]
            except KeyError:
            ...
            if len(coordnames) != self.dimension:
                ...
                print(
                    "Found Coordnames: %r(dimension = %s)"
        ...
        if self._parameterized:
            assert (
                self.indepvarname not in self.coordnames
            ...
            assert self.coordarray.shape[1] == len(self.indepvararray), (
                "Coord array length mismatch with independent variable" " array length"
            )
            # else:
        ...
        if self._parameterized:
            ...
            if not isincreasing(self.indepvararray):
                raise ValueError(
                    "Independent variable values must be in " "increasing order"
                )
    ...
    def reverse(self):
        ...
        self.labels.mapIndices(
            dict(zip(range(0, len(self)), range(len(self) - 1, -1, -1)))
        )
    ...
    def __setitem__(self, ix, p):
        ...
        if isinstance(ix, _int_types):
            if isinstance(p, Point):
                if compareNumTypes(self.coordtype, int32) and compareNumTypes(
                    p.coordtype, float64
                ):
            ...
            if ix == self.indepvarname:
                if isinstance(p, Pointset):
                    if compareNumTypes(self.indepvartype, int32) and compareNumTypes(
                        p.indepvartype, float64
                    ):
                        raise ValueError(
                            "Cannot update integer independent variable with a float"
                        )
                    if len(self) == len(p):
                        self.indepvararray = p.toarray()
                    else:
                        raise ValueError(
                            "Size mismatch for new independent variable array"
                        )
                    # labels ignored
                elif isinstance(p, dict):
                    if len(self) == len(p["c"]):
    ...
    def __getitem__(self, ix):
        ...
        if isinstance(ref1, (list, ndarray, _int_types)):
            ...
            raise TypeError(
                "Type %s is invalid for Pointset indexing" % str(type(ref1))
            )
        ...
        if self.checklevel == 0:
            ...
            try:
                ...
                if p - lval < self._abseps:
                    ...
                    rerr = rval - p
                    raise KeyError(
                        "%f not found in (%f, %f) @tol=%.16f: mismatches=(%.16f, %.16f)"
                        % (p, lval, rval, self._abseps, lerr, rerr)
                    )
    ...
    def __call__(self, p, coords=None):
        ...
        if isinstance(p, _seq_types):
            ...
            try:
                ...
                raise ValueError("Independent variable value not valid: " + str(p))
        if coords is None:
            if isinstance(ix, _int_types):
                label = self.labels[ix]
                try:
                ...
                return Pointset(
                    {
                        "coordarray": take(self.coordarray, ix, axis=1),
                        "coordnames": self.coordnames,
                        "indepvarname": self.indepvarname,
            ...
            if isinstance(ix, _int_types):
                ...
                return Point(
                    {
                        "coordarray": self.coordarray[clist, ix],
                ...
                try:
                    labels.mapIndices(
                        dict(zip(labels, [i - ix[0] for i in labels.getIndices()]))
                    )
                except AttributeError:
                    # empty
                    pass
                return Pointset(
                    {
                        "coordarray": take(self.coordarray[clist], ix, axis=1),
                        "coordnames": [self.coordnames[i] for i in clist],
                        "indepvarname": self.indepvarname,
                        "indepvararray": take(self.indepvararray, ix, axis=0),
                        "norm": self._normord,
            ...
            if self._parameterized:
                if isinstance(p, Point) and self.indepvarname in p.coordnames:
                    ...
                    if isinstance(tix, tuple):
                        ...
                        raise ValueError(
                            "Point at independent variable"
                            "value %f already present" % t
                        )
                elif (
                    isinstance(p, Pointset)
                    and p._parameterized
                    and p.indepvarname == self.indepvarname
                ):
                    ...
                    if (
                        len(intersect(self._ix_name_map, p._ix_name_map))
                        != self.dimension
                    ):
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Independent variable " "values in Pointset already present"
                        )
                    ...
                    raise TypeError(
                        "Inserted Point/Pointset must be "
    ...
    def append(self, parg, t=None, skipMatchingIndepvar=False):
        ...
        if isinstance(p, Pointset):
            ...
            if self._parameterized:
                if t is None:
                    ...
                    if len(tval) > 0 and tval[0] <= self.indepvararray[-1]:
                        ...
                        raise ValueError(
                            "Independent variable value too small to add pointset"
                        )
                    added_len = len(tval)
                else:
                    if t[0] == self.indepvararray[-1] and skipMatchingIndepvar:
                ...
                if t is not None:
                    raise TypeError(
                        "t argument cannot be used for non-parameterized pointsets"
                    )
            ...
            pdict = p.todict()
            self.coordarray[:, old_len:] = r_[
                [pdict[c][start_ix:] for c in self._ix_name_map]
            ]
            p_labels = copy(p.labels)
                ...
                if t is None:
                    if self.indepvarname not in p.coordnames:
                        ...
                        if tval <= self.indepvararray[-1]:
                            raise ValueError(
                                "Independent variable value too small to add Point"
                            )
                        pdim = pdim - 1
                else:
    ...
    def find(self, indepval, end=None):
        ...
        if not self._parameterized:
            raise TypeError(
                "Cannot find index from independent variable for "
...
class PointInfo(object):
        ...
        if ptlabels is None:
            ...
            for k, v in ptlabels.items():
                if not isinstance(k, _int_types):
                    raise TypeError(
                        "Initialization dictionary must be keyed " "by integer indices"
                    )
                if isinstance(v, str):
                    self.by_label[v][k] = {}
                    self.by_index[k][v] = {}
    ...
    def __eq__(self, other):
        try:
            return all(
                list(self.by_index.keys()) == list(other.by_index.keys())
            ) and all(list(self.by_label.keys()) == list(other.by_label.keys()))
        except AttributeError:
    ...
    def _infostr(self, tab=0):
        ...
        if lenself > 0:
            ...
            if lenself > 8:
                return (
                    basestr.join([_pretty_print_label(i) for i in entries[0:3]])
                    + ",\n"
                    + (tabstr + " .\n") * 3
                    + tabstr
...
def comparePointCoords(p1, p2, fussy=False):
    ...
    *not* compared.
    ...
    try:
        ...
        raise TypeError(
            "Invalid Points, Pointsets, or dictionaries passed " "to comparePointCoords"
        )
    test1 = alltrue([ks[0] == ks[1] for ks in zip(p1dk, p2dk)])
    test2 = alltrue(
        [vs[0] == vs[1] for vs in zip([p1d[k] for k in p1dk], [p2d[k] for k in p2dk])]
    )
    if fussy:
        try:
            ...
            raise TypeError(
                "Invalid Points, Pointsets, or dictionaries passed "
...
def pointsToPointset(
    pointlist, indepvarname="", indepvararray=None, indepvartype=float, norm=2
):
    ...
    if not paramd and indepvararray is not None:
        raise ValueError(
            "Must supply independent variable name for " "parameterized Pointset"
        )
    ...
    for p in pointlist:
        ...
        if coordnames == []:
            ...
            if paramd and indepvararray is None:
                assert indepvarname in coordnames, "Independent variable name missing"
...
def arrayToPointset(a, vnames=None, ia=None, iname=""):
    ...
    if vnames is None:
        ...
        if len(vnames) != shape(a)[0]:
            raise ValueError(
                "Mismatch between number of coordinate names and"
                " number of rows in array.\nCoordinates are "
                "assumed to be the rows of the array"
            )
    if ia is None:
        assert iname == "", (
            "Independent variable name must be none if no "
            "independent variable array provided"
        )
        return Pointset({"coordarray": a, "coordnames": vnames})
    else:
        if iname == "":
            iname = "t"
        return Pointset(
...
def exportPointset(
    ...
    """
    assert varvaldir in ["col", "row"], "invalid variable value write direction"
    ...
    try:
        ...
        raise TypeError(
            "Must pass Pointset to this function: use " "arrayToPointset first!"
        )
    infodict_usedkeys = []
    ...
    if ext != "":
        if ext[0] != ".":
            ext = "." + ext
    if append:
        assert varvaldir == "col", (
            "append mode not supported for row" "format of data ordering"
        )
        modestr = "ab"
    else:
        modestr = "wb"
    ...
    for fname, tup in infodict.items():
        ...
        try:
            if isinstance(tup, str):
                ...
                f.close()
                raise TypeError(
                    "infodict values must be singletons or "
...
def importPointset(xFileName, t=None, indices=None, sep=" ", preamblelines=0):
    ...
    if t is None:
        ...
        if len(tFileStrList) - preamblelines != filelen:
            raise ValueError(
                "Length of data and time files must be equal"
        ...
        if len(t) != filelen:
            raise ValueError(
                "Length of data file and t array must be "
    ...
    if indices == []:
        ...
        if get_t == 2:
            if t in indices:
                raise ValueError(
                    "You specified column " + str(t) + " as time "
    ...
    test_line = [
        n.strip('"').strip("'")
        for n in xFileStrList[preamblelines]
        .lstrip(sep)
        .lstrip(" ")
        .rstrip("\n")
        .rstrip("\r")
    ...
    for i in range(filelen - start):
        ...
        if get_t == 1:
            ...
            if len(tLine.split(sep)) != 1:
                raise ValueError("Only one t value expected per line of" " datafile")
            if tLine == "":
                continue
            tVals[i] = float(tLine)
        elif get_t == 2:
        ...
        if len(xLineVals) != dim:
            raise ValueError(
                "Exactly " + str(dim) + " values expected per " "line of datafile"
            )
    ...
    if get_t == 0:
        ...
        if varnames is None:
            ...
            return Pointset(
                indepvardict={t_name: tVals}, coorddict=dict(zip(varnames, xVals.T))
            )
...
def mergePointsets(pts1, pts2):
    ...
    if isparam1 and isparam2:
        assert (
            pts1.indepvarname == pts2.indepvarname
        ), "Parameterized pointsets must have identical independent variable names"
        assert all(
            pts1.indepvararray == pts2.indepvararray
...
def padPointset(pts, pinterval, value_dict, eps=None):
    ...
    if tlo < ts[0]:
        all_dict["t"] = tlo
        pts.insert(Point(coorddict=all_dict, labels="pad"))
        all_dict["t"] = ts[0] - eps
        pts.insert(Point(coorddict=all_dict, labels="pad"))
    if thi > ts[-1]:
        all_dict["t"] = ts[-1] + eps
