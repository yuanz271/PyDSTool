...
class MonotonyCriterion(object):
  ...
  """
    ...
    Computes the stopping criterion
    """
        value = state["new_value"] > state["old_value"] * (1.0 + self.error)
        if value:
...
class RelativeValueCriterion(object):
  ...
  """
    ...
    """
        ...
        value = (
            abs(new_value - old_value) / (new_value + old_value + 10e-127) < self.error
        )
        if value:
  ...
  """
    ...
    """
        ...
        value = (
            (
                self.weight
                * numpy.abs(new_parameters - old_parameters)
                / (new_parameters + old_parameters + 10e-127)
            )
            < self.error
        ).all()
        if value:
...
class AbsoluteParametersCriterion(object):
  ...
  """
    ...
    """
        ...
        new_parameters = state["new_parameters"]
        value = (
            (self.weight * numpy.abs(new_parameters - old_parameters)) < self.error
        ).all()
        if value:
